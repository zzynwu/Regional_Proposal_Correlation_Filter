function [bg_hist_new, fg_hist_new] = updateHistModel( patch, bg_area, fg_area, target_sz, norm_area, bg_hist, fg_hist, n_bins, colorImage, learning_rate_pwp)
...
%	assert(sum(pad_offset2==round(pad_offset2))==2, 'difference between bg_area and fg_area has to be even.');
    ...
    bg_hist_new = (1 - learning_rate_pwp) * bg_hist + learning_rate_pwp * computeHistogram(patch, bg_mask, n_bins, colorImage);
    fg_hist_new = (1 - learning_rate_pwp) * fg_hist + learning_rate_pwp * computeHistogram(patch, fg_mask, n_bins, colorImage);
