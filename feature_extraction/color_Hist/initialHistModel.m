function [bg_hist, fg_hist] = initialHistModel( patch, bg_area, fg_area, target_sz, norm_area, n_bins, colorImage)
...
%	assert(sum(pad_offset2==round(pad_offset2))==2, 'difference between bg_area and fg_area has to be even.');
    ...
    bg_hist = computeHistogram(patch, bg_mask, n_bins, colorImage);
    fg_hist = computeHistogram(patch, fg_mask, n_bins, colorImage);
